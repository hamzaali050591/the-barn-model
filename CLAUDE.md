...
**Timeline view (`ExecutionTimeline` + `TimelineCriticalPathCard`)** — 28-week / 6-phase GC buildout plan. Phases inlined as `TIMELINE_PHASES` near the SummaryCard. Each phase has a numbered header + colored phase band; tracks render as Gantt bars positioned globally as `% of TIMELINE_WEEKS` so concurrent trades stack vertically. Tones map to category palette (walnut = Cat 1/7/9, honey = Cat 2 MEP/4, sage = Cat 3/5 finishes, terracotta = Cat 6/8 + critical-path emphasis). Milestones render as terracotta diamonds (permits issued · rough-in inspection · fire marshal + health · soft open). `critical: true` adds a "CP" tag and feeds the dependency-gate card below the Gantt. Timeline is shared between V1 and V2 — sequencing is identical, only line-item costs differ. Scroll-fade-x affordance on mobile.
