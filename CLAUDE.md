...
- **Ramp is a distribution reserve, not a revenue ramp.** The hall is pre-leased from day 1; revenue and opex start at 100% the month a location opens. `rampMonths` simply delays distributions (builds operating cash reserve). **Side effect:** any cost or savings that lands inside the ramp window (months `openMonth` through `openMonth + rampMonths − 1`) is silently absorbed by the reserve and never reaches investor cash flow. With the post-Apr-30-2026 lease-holiday convention (clock starts at lease commencement = capital call month, not at open), a `l1LeaseHolidayMonths ≤ 3` is fully consumed by the buildout window before operations even start, so it has zero cash-flow effect at default; you only see IRR move once the holiday extends past `(buildoutMonths + rampMonths)` = 6, because earlier post-open months are still ramp-absorbed. This is conservative for investor returns by design — don't "fix" it without changing the ramp-as-reserve convention.
...
- **Annual escalators (added Apr 2026):** three independent compounding escalators on `ModelInputs`:
  ...
  - **Lease holiday convention (revised Apr 30 2026):** `l1LeaseHolidayMonths` waives **base rent only** (NNN is always charged — it's a real pass-through cost) for L1 only. **The holiday clock starts at lease commencement (= capital call month, `max(1, openMonth − 3)`), NOT at open.** This matches typical CRE practice where the lease begins when the tenant takes possession of the space for buildout, not when operations start. At default 3-mo buildout, a holiday of 0–3 months sits entirely inside the buildout window where no lease was being charged anyway, so the slider is inert. A holiday of 4+ months waives base rent during operating months, but those operating months still have to clear the ramp (`rampMonths`, default 3) before savings reach investor distributions — so practical IRR sensitivity only kicks in past `holiday > buildoutMonths + rampMonths` (= 6 at defaults). The escalator clock keeps ticking from open month regardless of holiday — Year 0 doesn't reset.
