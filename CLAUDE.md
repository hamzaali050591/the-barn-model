...
**Standing rule:** push every change finalized in a Claude Code session all the way through to Vercel before ending the task. Don't leave finalized work uncommitted or sitting on local `main`. The close-out sequence is always:

1. `git add` → commit on `main`
2. `git push origin main`
3. Vercel auto-deploys via `vercel.json` (~1–2 min rebuild on `origin/main`)

Direct pushes to `main` are protected by a permission rule and require a one-time per-push approval from the user (or the user runs `! git push origin main` themselves). The default cadence in this repo is direct push to `main` — PR flow is fine if the user explicitly prefers it, but never end a session with a finalized change unpushed.
...
- **Annual escalators (added Apr 2026):** three independent compounding escalators on `ModelInputs`:
  ...
  - **Lease holiday convention (revised Apr 30 2026 — upfront equity credit):** `l1LeaseHolidayMonths` is structured as a **landlord concession applied as an upfront equity credit on the L1 capital call**, NOT as a lease-line waiver. Mechanics: `l1HolidayCredit = min(investorEquityPerLocation, monthlyBaseRent × l1LeaseHolidayMonths)`. Computed in `capitalStack()` (engine.ts), exposed via `ModelOutputs.l1HolidayCredit`. The credit reduces the L1 capital call dollar-for-dollar at L1's commencement month (`max(1, openMonth − 3)`); investors literally write a smaller check. The lease itself (base rent + NNN) is charged in full from each location's open month onwards — there's no lease-line waiver in `masterLease`. This bypasses the ramp-as-reserve absorption convention entirely: every $1 of holiday flows directly to investor IRR. At default $26/PSF base rent × 9,180 sf, monthly base = $19,890, so each holiday month = ~$19.9K equity reduction. Credit is L1-only and one-time (does NOT multiply with `numLocations` in portfolio mode). Capped at L1's gross equity ask so the credit can't push the call negative.
...
**`/capex` (`src/pages/CapEx.tsx`)** — view of the Richmond V1 SOW with two orthogonal toggles. **Left-aligned: V1 / V2 toggle** (V1 = default; V2 = post-Niyi-Apr-29 revisions, further refined May 9 2026) controls budget data. **Right-aligned: Budget / Timeline toggle** (Budget = default) controls view mode — Budget shows the category breakdown, Timeline replaces it with the `ExecutionTimeline` Gantt. The two axes are independent. All numbers are **inlined as TypeScript constants** at the top of the file (cat1Items, cat2aItems...cat9Items for V1; cat1ItemsV2 ... cat9ItemsV2 for V2 — V2 currently initializes as a deep clone of V1 via `.map(i => ({ ...i }))` and diverges per line item by replacing the clone with explicit literals or surgical `.map` overrides). Why inline rather than data-driven: the SOW is the deal artifact, not a model input. The `computeTotals()` factory takes a categories array + cat9 items and returns a `CapExData` object (hardCosts, softCosts, contingency, totalProjectCost, tiAllowance, netEquity, capexPSF) — `V1_DATA` and `V2_DATA` are precomputed at module scope and the active one is passed to `SummaryCard` / `CategoryCard`. **`SummaryCard` derives the contingency rate dynamically from `data.contingency / data.hardCosts`**, so V1 renders "Contingency (10% of Hard Costs)" and V2 renders "Contingency (8% of Hard Costs)" without per-version branching. Cat 2 (MEP) is the only category with sub-categories (2a HVAC → 2g Low-Voltage); other categories use a flat line-item list. Includes the gas service upgrade callout and Critical Path sequencing block — both are demo-grade copy, edit carefully.

**V1 vs V2 deltas (current):** V1 = $1.774M total ($193/PSF) at 10% contingency. V2 = $1.581M total ($172/PSF) at 8% contingency. Seven V2 changes vs V1: Cat 1 collapsed to $10K rolled-up line · Cat 2a (HVAC/BMS) RTUs $170K→$150K, MAU $45K→$15K, BMS $25K→$20K, Restroom exhaust $8K→$6K · Cat 2c (Electrical) scaled UP from $128K→$140K (line items proportionally ×1.094, rounded to $1K) · Cat 2d (Plumbing) scaled DOWN $129K→$110K · Cat 4 food stalls $40K→$35K/stall and 4 non-food kiosks consolidated into single $80K line · Cat 9 architecture+MEP combined into single $40K fee · Cat 9 contingency dropped from 10% to 8% of hard costs. The `V2ChangesSummary` component (rendered in V2 mode) is the source of truth for explaining these to investors — keep its row table aligned with the actual constants. **V2 contingency formula is `Math.round(V2_HARD_COSTS * 0.08)` and recomputes automatically if any V2 hard-cost line item changes.** Cat 9 in BOTH versions includes a $10K operator-led "Pre-development groundwork" line (with 2 sub-items: $7K concept/modeling/design/renderings/OpEx/CapEx scope work + $3K initial vendor sourcing) that pairs with a $30K Construction Management line — together $40K of operator effort from concept through launch day. This is shared baseline scope, NOT a V2 revision; do not add it to `V2ChangesSummary`. Sub-line items render inside the parent row's expanded view via the optional `subItems` field on `LineItem` — add more by following the same pattern (parent cost = sum of children).

**Timeline view (`TimelineSummaryCard` + `ExecutionTimeline` + `TimelineCriticalPathCard`)** — 22-week / 3-macro-phase end-to-end plan (`TIMELINE_WEEKS = 22`, header rendered as 11 × 2-week column groups via `TIMELINE_HEADER_COLS = 11`). Three phases: **Architectural Drawings** (4 weeks, W1–4) → **Permitting** (8 weeks, W5–12) → **Construction** (10 weeks, W13–22). The `TimelineSummaryCard` renders ABOVE the Gantt as a quick honey-tinted bullet list of those three durations + scope summary — it's the first thing investors see in Timeline view. Phases inlined as `TIMELINE_PHASES` near the SummaryCard. Each phase has a numbered header + colored phase band; tracks render as Gantt bars positioned globally as `% of TIMELINE_WEEKS` so concurrent trades stack vertically. Tones map to category palette (walnut = Cat 1/9/Vendors, honey = Cat 2 MEP/4, sage = Cat 3/5 finishes, terracotta = Cat 6/8 + critical-path emphasis). Milestones render as terracotta diamonds (drawings stamped · permits issued · rough-in inspection · fire marshal + health · soft open). `critical: true` adds a "CP" tag and feeds the dependency-gate card below the Gantt. Timeline is shared between V1 and V2 — sequencing is identical, only line-item costs differ. Min-width on the Gantt grid is **1080px** (raised from 820px to give 11 columns breathing room); below `md` the page horizontal-scrolls with `scroll-fade-x`. **Cat 7 (The Barn App) is intentionally NOT on the timeline** — app dev runs as software work in parallel with the buildout, not as a GC trade; it lives only in the Budget view. If the buildout timeline ever needs to lengthen or the phase split changes, adjust `TIMELINE_WEEKS` + `TIMELINE_HEADER_COLS` (must divide evenly into TIMELINE_WEEKS) + per-track `startWeek`/`durationWeeks` + the `TimelineSummaryCard` rows + `TimelineCriticalPathCard` week references all together.
