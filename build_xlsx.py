#!/usr/bin/env python3
"""
Generate The Barn — Financial Model.xlsx from scratch to mirror the web app.
...
SQFT, TI_PSF, CAPEX_PSF = 9180, 35, 170
BASE_RENT_PSF, NNN_PSF = 26, 9.5
GP_INVESTMENT = 200_000
DEBT_PER_LOC, DEBT_RATE_PCT, DEBT_TERM_YEARS = 0, 0, 10
NUM_LOCATIONS, EXIT_MULTIPLE = 7, 4
RAMP_MONTHS, L1_HOLIDAY = 3, 0
OPEN_SCHEDULE = [4, 16, 20, 24, 28, 32, 36]
HOLD_MONTHS = 48
RENT_ESC, OPEX_ESC = 4, 3
BASE_RENT_ESC, NNN_ESC = 2, 2
SALARY_BASE, SALARY_STEP, PROFIT_SHARE = 84_000, 20_000, 5
REV_MODEL = "base"
...
VENDORS = [("Food Vendors", 8, 6500, 35000, True),
...
notes = [
    "",
    "This workbook is a faithful Excel reproduction of the web app's runModel engine.",
    "  • Source of truth for defaults: src/utils/types.ts → DEFAULT_INPUTS",
    "  • Source of truth for formulas: src/utils/engine.ts → runModel",
    "",
    "All inputs live on the Inputs sheet (light-cream cells). Edit them; everything else recomputes.",
    "",
    "Sheet roadmap:",
    ...
    "  Debt Schedule    — fixed-term amortization per location + balloon balance at exit",
    "  Monthly Cash Flow— 72 months × per-location detail + portfolio aggregation",
    ...
    "  • Per-location annual escalators apply on each location's open clock",
    ...
    "",
    "Backup of the prior xlsx is saved alongside this file with a date-stamped filename.",
]
...
setw(ws, [22] + [13]*7)
ws['A1'] = "DEBT SCHEDULE — fixed amortization term per location; balloon at exit if term > hold"; ws['A1'].font = Font(bold=True, size=12, color="5C4033")
...
# Total balloon payoff at exit — sum of per-location balances row
named(wb, "debtPayoff", "'Debt Schedule'!$B$11:$H$11")
...
# Net equity = MAX(0, gross exit - debt balloon). Promote applied to net.
for m in range(1, MAX_MONTHS + 1):
    r = m + 1
