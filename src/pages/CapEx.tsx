  ...
  return (
    <div className="border-b border-walnut/5 last:border-b-0">
      <button
        ...
        className="w-full flex items-start gap-3 px-3 py-2 text-left hover:bg-honey/5 transition-colors cursor-pointer"
      >
        <span className="text-[10px] font-mono text-walnut-light w-5 shrink-0 mt-0.5">{item.num}</span>
        <span className="text-xs text-walnut flex-1 min-w-0 font-medium">{item.name}</span>
        <span className="text-xs font-bold text-walnut tabular-nums shrink-0">{fmtDollarFull(item.cost)}</span>
        <span className={`text-walnut-light text-[10px] mt-1 transition-transform shrink-0 ${open ? 'rotate-180' : ''}`}>&#9662;</span>
      </button>
      {open && (
    ...
    <div className={`glass rounded-2xl transition-all ${open ? '!shadow-lg !shadow-honey/10' : ''}`}>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="font-bold text-walnut text-sm">{cat.title}</div>
                ...
                return (
                  <div key={sub.key} className="bg-white/40 rounded-lg border border-walnut/5">
                    ...
                    >
                      ...
                      <span className={`text-walnut-light text-[10px] transition-transform shrink-0 ${subOpen ? 'rotate-180' : ''}`}>&#9662;</span>
                    </button>
                    {subOpen && (
