...
export default function SubsNavBar({ current }: Props) {
  ...
  return (
    <header className="nav-glass text-cream sticky top-0 z-50 border-b border-cream/5">
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        >
          The Barn <span className="text-honey/80 text-sm font-medium">· Subcontractors</span>
        </button>
          ...
          {links.map(link => (
            <button
              ...
              className={`group relative px-3 py-1.5 rounded-lg text-sm font-medium transition-all duration-300 cursor-pointer ${
                current === link.path ? 'text-honey' : 'text-cream/70 hover:text-cream'
              }`}
            >
              <span className={`absolute inset-0 rounded-lg transition-all duration-300 ${
                current === link.path ? 'bg-cream/12' : 'bg-transparent group-hover:bg-cream/8'
              }`} />
              <span className="relative">{link.label}</span>
              <span className={`absolute left-3 right-3 bottom-1 h-px transition-all duration-300 ${
                current === link.path ? 'bg-honey/70 scale-x-100' : 'bg-honey/0 scale-x-0 group-hover:bg-honey/40 group-hover:scale-x-100'
              }`} />
            </button>
          ))}
        </nav>
